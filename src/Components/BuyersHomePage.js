  ...
  const theProperties = sortedProperties.map((house) => {
    return (
      <div key={house.id} className="property">
        ...
        <p className="address">
          <em onClick={() => handleAddressClick(house.address)} className="address-link">
        ...
        <div className="modal-wrapper">
          <button onClick={() => toggleModal(house)}>Sellers Information</button>
          {selectedProperty === house && (
            <div className="modal">
              <div className="modal-content">
                <span className="close" onClick={closeModal}>&times;</span>
                <p>Name: {house.seller.name}</p>
                <p>Email: <a href={`mailto:${house.seller.email}`} className="email-link">{house.seller.email}</a></p>
